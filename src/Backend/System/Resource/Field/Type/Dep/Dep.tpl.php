...
if (in_array($mode, ['list', 'detail'])) {
    $dep_link = $this->service(\App\Backend\System\Resource\Pilot\ResourcePilotRegistry::class)->__call($deptype)->link('index');
    $dep_param = $dep_param ?? $reltype . '_id';
    $dep_url = $this->alterUri($dep_link['url'], [$dep_param => $value['id']]);
    $dep_count = $this->service(\App\Backend\System\Resource\Repository\ResourceRepositoryRegistry::class)->__call($deptype)->count([$dep_param => $value['id']]);
}
?>
